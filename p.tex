...
{\{timothy.jone, ahmed.mourad, kevin.ong, falk.scholer, evi.yulanti\}@rmit.edu.au}
}}
...
\begin{itemize}
  \item \method{word2vec}:  In the former method, we took a pre-trained word
...
The LiveQA challenge result is given in Table~\ref{tab:runs}, where our
submitted runs and the average result across all runs are included.  Our base
run RMIT0 delivered the best performance in our experiment, achieving 0.663
{in} Avg Score.  While the base run appears to outperform the average across all 
runs submitted to the challenge, other improvements we made
all led to decreased performance.  

Limiting the output to only the first sentence in the summary (RMIT1) appears
to have a negative effect on precision at all relevance level.
